...
async function genImagen(_imageBase64: string, prompt: string): Promise<{ image_base64: string; provider: string }> {
  ...
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/imagen-4:generateImages?key=${key}`, {
    ...
    body: JSON.stringify({ prompt: `${prompt}. Photorealistic 4K.`, config: { numberOfImages: 1, outputOptions: { mimeType: 'image/png' } } }),
  })
  if (response.status === 429) throw new Error('RATE_LIMITED')
...
async function genHuggingFace(_imageBase64: string, prompt: string): Promise<{ image_base64: string; provider: string }> {
  ...
  const response = await fetch('https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-xl-base-1.0', {
    ...
    body: JSON.stringify({ inputs: `${prompt}. Photorealistic 4K fashion photograph.` }),
  })
...
async function processZoomShots(jobId: string, jewelryImageUrl: string, projectId: string, imageId: string, userId: string) {
  ...
  try {
    ...
    for (let i = 0; i < ZOOM_SHOTS.length; i++) {
      ...
      if (storageError) throw new Error(`Storage failed ${angle}: ${storageError.message}`)

      const { data: publicUrlData } = supabase.storage.from('project-images').getPublicUrl(storagePath)
      await supabase.from('project_images').insert({
        project_id: projectId, storage_url: publicUrlData.publicUrl, type: 'zoom',
        metadata: { angle, resolution: '4K', jewelry_image_id: imageId },
      })
      await supabase.from('processing_jobs').update({ progress: Math.round(((i * 2 + 2) / (ZOOM_SHOTS.length * 2)) * 100) }).eq('id', jobId)

      if (i < ZOOM_SHOTS.length - 1) await sleep(INTER_REQUEST_DELAY)
    }
